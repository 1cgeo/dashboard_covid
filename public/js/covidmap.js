...
=======
  ...
  connectEvents() {
    this.map.on("click", () => {
      this.closeSidebar();
      if (
        this.getCurrentPopoverLayer() &&
        !this.getCurrentPopoverLayer().eventWasReceived()
      ) {
        this.zoomToDefaultBounds();
        this.map.closePopup();
        this.map.setZoom(4);
  ...
  create(options) {
    return L.map(options.elementId, {
      minZoom: 3,
      zoomControl: false,
      maxBounds: this.options.maxBounds,
    })
  ...
  createLayersButtons(buttons, cb) {
    ...
    var buttonsDiv = $("<div></div>");
    buttons.forEach(function (elem, idx) {
      var div = $("<div></div>").attr("class", "custom-control custom-radio");
      var input = $("<input></input>")
        .attr("id", elem.id)
        .attr("type", "radio")
        .attr("class", "custom-control-input")
    ...
    $("input[name='layer']").change(function (e) {
      cb($(this).val());
    });
  }
  ...
  loadThemeLayer(themeLayerId) {
    this.map.closePopup();
    var themeOptions = this.getCurrentLayerOptions().themeLayers.find(
      (theme) => {
        return theme.id == themeLayerId;
      }
    );
    ...
    themeOptions.map = this;
    var factories = new Factories();
    this.currentThemeLayer = factories.createLayer(
      themeOptions.type,
      themeOptions
    );
  }
...
>>>>>>> fab01799f82f8bee1a1b004b199baacef1ab0e39
}
