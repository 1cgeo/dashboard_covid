//Adaptado para ES5 de https://github.com/fungbo/epidemiological-week
...
exports.calculate = function (dateStr, format = "YYYY-MM-DD") {
  ...
  if (date.isSameOrAfter(firstEpiWeekDay)) {
    ...
    if (firstEpiWeekDay.dayOfYear() === 1) {
      duration =
        date.dayOfYear() - firstEpiWeekDay.dayOfYear() + firstEpiWeekDay.day();
    } else {
    ...
    if (week > MAX_EPI_WEEKS_IN_ONE_YEAR) {
      ...
      return firstDayNextYear.isSame(firstEpiWeekDayNextYear)
        ? { year: year + 1, week: 1 }
        : { year: INVALID_VALUE, week: INVALID_VALUE };
    }
